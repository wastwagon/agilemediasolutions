...
/** `/newhomepage` stats bento — defaults from `SITE_SECTION_CODE_DEFAULTS['newhomepage.whyUsBento']` (Site Content overrides). */
export function getNewHomepageWhyUsBentoDefaults(): Record<string, string> {
  return { ...(SITE_SECTION_CODE_DEFAULTS['newhomepage.whyUsBento'] ?? {}) };
}

/** `/newhomepage` FAQ rows — defaults from `SITE_SECTION_CODE_DEFAULTS['newhomepage.faq']` (Site Content overrides). */
export function getNewHomepageFaqDefaults(): Record<string, string> {
  return { ...(SITE_SECTION_CODE_DEFAULTS['newhomepage.faq'] ?? {}) };
}
